...
def make_tree(char_list):
    ...
    for char in char_list:
        char_number = char[1]
        tree_dict[char_number] = Tree((), char_number)

    while len(char_list) > 2:
        weight_1, char_number_1 = heappop(char_list)
        weight_2, char_number_2 = heappop(char_list)
        heappush(char_list, (weight_1 + weight_2, tree_id))
        tree1 = tree_dict[char_number_1]
        tree2 = tree_dict[char_number_2]
        tree_dict[tree_id] = Tree((tree1, tree2))
        tree_id += 1

    char_number_1 = heappop(char_list)[1]
    char_number_2 = heappop(char_list)[1]

    return Tree((tree_dict[char_number_1], tree_dict[char_number_2]))
